void retrieveMemory() 
{
    ...
    program[0].active = EEPROM.readBool(address[4]);
    ...
    program[1].active = EEPROM.readBool(address[9]);
}
...
void setMemory() 
{
    ...
    if (program[0].active != EEPROM.readBool(address[4]))    { EEPROM.put(address[4], program[0].active); }
    ...
    if (program[1].active != EEPROM.readBool(address[9]))    { EEPROM.put(address[9], program[1].active); }
    EEPROM.commit();
}